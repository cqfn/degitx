...
The solution is a distributed Git repository manager with
strong consistent replica nodes. It guarantees data consistency across
different regions, durability by replicating repositories on different server racks,
read scalability by routing fetch traffic to different nodes with independend disks hardware.

DeGit is a peer-to-peer system, no persistent master nodes exists.
It uses overlay network built on transport network layer and based on persistent node locators to communicate
with each other, see \hyperref[sec:locators]{Locators}. Nodes use \hyperref[sec:discovery]{Discovery} protocol
to find network address of another node.

Each node may have zero or more git repository replicas. To find node locator of certain repository,
node uses \hyperref[sec:metadata]{Metadata} exchange protocol. When the system decides to start managing a replica
on soe node, it updates metadata by mapping repository id with node locator.

When client write git data to system (via \code{push}), the request is routing to one of the replicas
which starts leader election and updates the replicated log of consensus of repository holders (replicas),
see \hyperref[sec:data]{Data} exchange protocol.

Each node exposes \hyperref[sec:api]{RPC API}, clients uses it to perform git operations on distributed repository
data. Client doesn't know if the node contains certain repository it just sends the read or write request
to the node, and node degates this operation to on of repository replicas to perform it.

The system collects usage statistics about fetch and push operation for each node and repository,
and may decide to move repositories to most frequently used nodes based on this statistics.
...
\begin{description}
  ...
  \item[Data] - Git objects and references exchange protocol, commands to add new objects and update references.
\end{description}
...
for node lookup and could be used together:
...
\section{Explanation}
\subsection{How does DeGitX update repository}
This is how DeGitX system updates repository: a client \code{C} uses RPC API
to push git data to repository \code{R}.
The RPC request comes to one of DeGitX nodes \code{N1}.

\begin{figure}[H]
\centering
  \begin{tikzpicture}
    \node[block] (C) {Client C};
    \node[block, right=of C] (N1) {Node N1};
    \path[ln] (C) edge node {push R1} (N1);
  \end{tikzpicture}
  \caption{Client C1 push data to repository R1 via node N1}
\end{figure}

The node \code{N1} uses \hyperref[sec:metadata]{metadata} to find \hyperref[sec:locators]{locator} \code{L2}
of any replica of repository requested.
With this locator, node performs lookup for node
with locator using \hyperref[sec:discovery]{discovery protocol} to find \hyperref[sec:network]{real address}
\code{A2} of node with locator which replicates the repository requested.
...
\centering
  \begin{tikzpicture}
    \node[block] (C) {Client C};
    \node[block, right=of C] (N1) {Node N1};
    \node[block, below=of C] (N2) {N2, L2, A2};
    \node[block, below=of N1] (N3) {N3, L3, A3};
    \node[block, below=of N3] (N4) {N4, L4, A4};
    \node[fit=(N2)(N3)(N4), draw, dashed](R1){Replicas of R1};
    \path[ln] (C) edge node {push R1} (N1);
    \path[ln] (N1) edge node {knows A3} (N3);
  \end{tikzpicture}
  \caption{N1 find address of replica N3 of repository R1}
\end{figure}

After that node knows real address where repository is located.
The node sends internal RPC call to \code{A3} and asks to update the repository with requested changes.
Node \code{N3} elect the leader for these operation across \code{N3}, \code{N4} and \code{N5} nodes.
The leader \code{L4} adds new operation request to the state machine log for repository
and replicates it across consensus of repository holders (across all replicas in best case).
...
\centering
  \begin{tikzpicture}
    ...
    \node[block, below=of C] (N2) {N2, L2, A2};
    \node[block, below=of N1] (N3) {N3, L3, A3};
    \node[block, below=of N3] (N4) {N4, L4, A4 (leader)};
    \node[fit=(N2)(N3)(N4), draw, dashed](R1){};
    \path[ln] (C) edge node {push R1} (N1);
    \path[ln] (N1) edge node {requests update of R1} (N3);
    \path[ln] (N4) edge node {update log} (N3);
    \path[ln] (N4) edge node {update log} (N2);
  \end{tikzpicture}
  \caption{Replica N3 initiates leader election, the node N4 becomes the leader, and replicate update across all replicas of R1}
\end{figure}

After successful update of \code{R1} node \code{N2} returns with success status to node \code{N1},
and node \code{N1} sends success to git client \code{C}.

\subsection{How does DeGitX fetches the repository}
Fetching repository is easier comparing to pushing. First steps are similar to write: client
sends fetch requrest to any node in a system for repository. Then node look up for locator of
any replica with requesting repository. Then node finds real address of this locator.
Knowing address of repository, it reads git data from this replica and returns it to the client.
If replica fails to respond, it tries another replica.

\todo{add more topics here}
