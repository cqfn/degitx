...
DeGitX project aims to fix few different projects bot technical and conceptual
of source code repository management.

\subsection{Scalability}

One of the problem is scalability. Big company or developer's community may have
millions of repository distributed around the world, and teams located in different countries
(regions). These repositories are accessed by different ways:
\begin{description}
...
This repository management system can be distributed and clients will be routed to storage with
correct repository location, but on huge amount of \code{fetch} traffic for one repository,
the disk read requests may go above IOPS limit and repository storage will stop serving following requests.

There was an attempt to implement POC distributed repository manager with
eventually consistency guarantee: one repository was replicated asynchronously after update to multiple storages,
and clients were load balanced to different repository storages.
...
CI build immediately which clones the repository. CI was able to clone only
"master" repository because the replication was not completed before CI cloning started.
...
and lead to high fetch traffic to primary repository storage, where all push operations were handled.
It causes same problems as not replicated system - the primary node goes beyond IOPS limits and
rejects new fetch requests.
...
It consists of two parts, see Figure ~\ref{fig:comp-arc-overview}:

\begin{figure}
  \begin{center}
    \begin{tikzpicture}
      \matrix[row sep=10mm, column sep=10mm](clients){
        \node[block] (C1) {C$_{1}$}; \\
        \node[block] (C2) {C$_{2}$}; \\
        \node[block] (CN) {C$_{n}$}; \\
      };
      \matrix[row sep=10mm, column sep=10mm, right=of clients](load-balancers){
        \node[block] (LB1) {Lb$_{1}$}; \\
        \node[block] (LB2) {Lb$_{2}$}; \\
        \node[block] (LBN) {Lb$_{n}$}; \\
      };
      \matrix[row sep=10mm, column sep=10mm, right=of load-balancers](nodes){
        \node[O] (N1) {N$_{1}$}; \\
        \node[O] (N2) {N$_{2}$}; \\
        \node[O] (N3) {N$_{3}$}; \\
        \node[O] (N4) {N$_{4}$}; \\
  ...
  \caption{
    ...
    Each client C$_{x}$ connected one-to-one to DeGit load balancer Lb$_{x}$,
    load balancer proxies all requests to storage nodes N$_{1..n}$.
  }
...
\begin{figure}
  \begin{center}
    \begin{tikzpicture}
      ...
      \node[block, below=of browser] (git) {git};

      \node[block, right=of browser] (web) {Web};
      \node[block, below=of web] (shell) {Shell};
      \node[block, below=of shell] (workhorse) {Workhorse};
      \node[fit=(shell)(workhorse)(web), draw, dashed, label={[right=0cm, above=3cm]1cm:GitLab set}](gl-set){};

      \node[block, right=of shell, xshift=1cm] (fe) {DeGit front-end};

      \draw[-] (git.east) -- node {ssh} (shell.west);
      \draw[-] (git.east) -- node {http(s)} (workhorse.west);
      \draw[-] (browser.east) -- node {API} (web.west);
      \draw[-] (shell.east) -- node {gRPC} (fe.west);
      \draw[-] (workhorse.east) -- node {gRPC} (fe.west);
      \draw[-] (web.east) -- node {gRPC} (fe.west);
    \end{tikzpicture}
  \end{center}
  \caption{
...
The communication layer of DeGitX front-end and back-end is DeGit metadata: it consists of mapping
...
\begin{description}
  \item[LPD] - local peer discovery: each peer sends UDP messages with locator ID and stored repository hashes
    via UDP broadcasts to the local network periodically, it updates locator ID to address maps and optimizes
    metadata exchange (\ref{sec:metadata}) protocol. It's cheap and fast for network layer (due to UDP messages),
    the main goal is to update peers and load balancers tables in same region as peer node.
...
\subsection{Metadata}
\label{sec:metadata}
DeGitX metadata is a relation of repository hash to node locator ID. It's how metadata is updated:
\begin{enumerate}
  \item A new node wants to replicate some repository (e.g. node has predefined setting to store some repository name)
