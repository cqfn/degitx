  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \matrix[row sep=10mm, column sep=10mm, right=of clients](load-balancers){
        \node[block] (FE1) {Fe$_{1}$}; \\
        \node[block] (FE2) {Fe$_{2}$}; \\
        \node[block] (FEN) {Fe$_{n}$}; \\
      };
      \node[fit=(C1)(FE1), draw, dashed](vm1){};
      \node[fit=(C2)(FE2), draw, dashed](vm2){};
      \node[fit=(CN)(FEN), draw, dashed](vmn){};
...
\begin{enumerate}
  \item A new node wants to replicate some repository (e.g. node has predefined setting to store some repository)
