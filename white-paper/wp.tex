...
\item[scalability] --- one repository may be used actively by many users simultaneously. Storage disk
  ...
  solve this problem.~\todo{REF find references about storage scaling to improve read capacity in a distributed system.}
...
eventual consistency with strong consistency in the context of bias between Read and Write operations in
...
since they use a similar approach for introducing node IDs in the P2P system}.
...
\begin{description}
  ...
  \item[Distributed DB] --- network addresses could be propagated to the system using a supplementary distributed
    ...
    \todo{REF add cites for comparing distributed key-value database in the context of metadata storage for cross-region
    access}.
...
\begin{figure}
  \begin{center}
    \begin{tikzpicture}[
        ...
        group-node/.style={rectangle, draw=gray, inner sep=0.4cm}
      ]
      %    FE            BE
      % [  lb  ]    [git] [hook]
      % [  tm  ]    [    rm    ]
      ...
      \draw[->]         ($(hook.south)+(-0.4,0)$)       -- node[left] {4}       ++(0,-0.6);
      ...
      \draw[->]         ($(hook.south)+(0.4,-0.6)$)     -- node[right] {11}     ($(hook.south)+(0.4,0)$);
      \draw[->]         ($(hook.west)+(0,-0.2)$)        -- node[below] {12}     ($(git.east)+(0,-0.2)$);
      \draw[->]         ($(rm.west)+(0,0.3)$)          ..
            controls ($(rm.west)+(-0.7,0.7)$) and ($(tm.east)+(0.7,0.7)$)
                                                        .. node[above] {13}     ($(tm.east)+(0,0.3)$);
      \draw[->]         (tm.west)                       -- node[above] {14}     ($(tm.west)+(-1.6,0)$);
    \end{tikzpicture}
  \end{center}
  \caption{
    ...
    \adjustbox{scale=0.5,center}{%
      \begin{tikzpicture}
        \begin{umlseqdiag}
          ...
          \umlobject[no ddots]{LB}
          \umlobject[no ddots]{TM}
          \umlobject[no ddots]{git}
          \umlobject[no ddots]{hook}
          \umlobject[no ddots]{RM}
          \umlobject[no ddots]{Proposer}
          \umlobject[no ddots]{Acceptor}
          \begin{umlcall}[type=asynchron, op=1]{CLI}{LB}
            \begin{umlcall}[op=2]{LB}{git}
            \end{umlcall}
            \begin{umlcall}[op=3, return=12]{git}{hook}
              \begin{umlcall}[op=4, type=asynchron]{hook}{RM}
                \begin{umlcall}[op=5, type=asynchron]{RM}{Proposer}
                  ...
                  \end{umlcall}
                \end{umlcall}
                \begin{umlcall}[op=8]{RM}{Acceptor}
                \end{umlcall}
              ...
              \end{umlcall}
            \end{umlcall}
            \begin{umlcall}[op=13, type=asynchron, dt=7]{RM}{TM}
            \end{umlcall}
            \begin{umlcall}[op=14, type=asynchron]{TM}{CLI}
            \end{umlcall}
          \end{umlcall}
        \end{umlseqdiag}
      \end{tikzpicture}
