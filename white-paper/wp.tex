...
\begin{description}
  \item[LPD] - local peer discovery: each peer sends UDP messages with locator ID.
    ...
    BitTorrent uses following multicast groups: A) 239.192.152.143:6771 (org-local) and B) [ff15::efc0:988f]:6771 (site-local) for Bep 14 implementation.
    They've chosen such IPs because 239.192.0.0/14 is defined to be the IPv4 Organization Local Scope,
    and is the space from which an organization should allocate sub-
    ranges when defining scopes for private use.
    ff15::efc0:988f also comes from \href{http://tools.ietf.org/html/rfc4291#section-2.7}{IPv6 spec} and means:
    \begin{itemize}
      \item[--] FF == Multicast
...
\begin{figure}
  \begin{center}
    \begin{tikzpicture}
      \begin{umlseqdiag}
        \umlcontrol[no ddots]{Client}
        \umlobject[no ddots]{FE}
        \umlmulti[no ddots]{BE}
        \umldatabase[no ddots]{Meta}
        \begin{umlcall}[op={1: push(r,c)}, return=OK]{Client}{FE}
          \begin{umlcall}[op={2: lookup(r)}, return={L[]}]{FE}{Meta}
          \end{umlcall}
          \begin{umlcall}[op={3: l.push(r,c)}, return=OK]{FE}{BE}
            \begin{umlcallself}[op={4: leader-election}]{BE}
            \end{umlcallself}
            \begin{umlcallself}[op={5: git data update}]{BE}
            \end{umlcallself}
          \end{umlcall}
        \end{umlcall}
      \end{umlseqdiag}
