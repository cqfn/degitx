...
\item[scalability] --- one repository may be used actively by many users simultaneously. Storage disk
  ...
  solve this problem.~\todo{REF find references about storage scaling to improve read capacity in distributed system.}
...
clients were load balanced to different git repository storages.~\todo{REF find references about comparing
eventually consistency with strong consistency in context of bias between read and write operations in
...
as described in~\todo{REF add cite to "A Practicable Approach Towards Secure Key-Based Routing"}.
...
\todo{REF cite IPFS research
https://raw.githubusercontent.com/ipfs-inactive/papers/master/ipfs-cap2pfs/ipfs-p2p-file-system.pdf
since they uses similar approach for introducing node IDs in P2P system}.
...
\begin{description}
  \item[LPD] --- local peer discovery: each peer sends UDP messages containing the locator ID.
    ...
    There are two common ways to implement LDP: \href{http://bittorrent.org/beps/bep_0014.html}{bep 14} and \href{http://bittorrent.org/beps/bep_0026.html}{bep 26}.~\todo{REF use cites to bittorent and zeroconf researches instead of URL links here}.
  ...
  \item[Distributed DB] --- network addresses could be propagated to the system using a supplementary distributed
    ...
    \todo{REF add cites for comparing distributed key-value database in context of metadata storage for cross-region
    access}.
  ...
  \item[DHT] --- distributed hash tables use different metrics to compare the distance of nodes in the overlay network.
    ...
    contains node addresses with the same ID prefix.~\todo{REF add cites to Kademlia and Chord DHT papers.}
\end{description}
...
CAs could solve security issues, since according to the~\todo{REF insert a cite for
``Secure routing for structured peer-to-peer overlay networks'' paper}, all attacks are based on the presence of malicious nodes.
...
so two-phase commit protocol (2PC)~\todo{REF add references for two-phase commit protocol},
three-phase commit protocol (3PC)~\todo{REF add reference for three-phase commit protocol},
or Paxos-commit~\cite{consensus-on-transaction-commit}
much better than Raft or Paxos here.
Git itself is also a system for which Raft or Paxos doesn't really fit well.
It's unclear how to implement log and local state machine over git.
In this algorithms replicas should apply any value proposed by leader, but git could deny some updates.
An atomic commit algorithms care exactly about this problem and it includes voting, which allow git servers to check if they can apply proposed value.
...
so it is possible to handle multiple updates to different independent sets of refs inside one repo at the same time.
It means that several branches could be updated parallel without locking each other.

Transaction starts right before a user attempts to write new refs.
...
data storage~\todo{REF add link to the patent ``Content-addressable data storage'' number US009830345B1}
...
load balancing hor high load network traffic and correlation with uniform distribution of storage access}.
