...
(* \ensuremath{MIT} License Copyright (c) 2020 \ensuremath{CQFN
}%
\end{cpar}%
\begin{cpar}{0}{T}{F}{2.5}{0}{}%
* https:\ensuremath{\.{\slsl}github.com}/cqfn/degitx/blob/master/LICENSE
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
*
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 * The module shows a conflict in the 3-phase commit protocol with the backup
 transaction manager.
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 * The problem lies in the different behavior of the Prime and Second (Backup)
 transaction managers.
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 * The \ensuremath{3PC} protocol guarantees consistency if and only if ONE and
 \ensuremath{{\ONLY}} \ensuremath{TM} makes decisions.
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 * at the moment at any stage of processing. And all nodes are available for
 this \ensuremath{TM}.
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
*)
\end{cpar}%
\end{lcom}%
\@x{}\moduleLeftDash\@xx{ {\MODULE} BackupCol}\moduleRightDash\@xx{}%
\@x{ {\EXTENDS} Sequences ,\, FiniteSets ,\, TLC}%
\@pvspace{8.0pt}%
\@x{ {\CONSTANTS} RM}%
\@y{\@s{0}%
 Set of all \ensuremath{Resourse} Managers. All available for all
 \ensuremath{TMs
}}%
\@xx{}%
\@pvspace{8.0pt}%
...
\@x{\@s{16.4} rmState}%
\@y{\@s{0}%
 Current state of every particular \ensuremath{RM} (set)
}%
\@xx{}%
\@pvspace{8.0pt}%
\@x{\@s{16.4} isAnyAborted ( RMS ) \.{\defeq}}%
 \@x{\@s{32.8} \.{\land} \E\, r \.{\in} RMS \.{:} rmState [ r ]
 \.{=}\@w{aborted}}%
\@pvspace{8.0pt}%
\@x{\@s{16.4} isAnyWorking ( RMS ) \.{\defeq}}%
 \@x{\@s{32.8} \.{\land} \E\, r \.{\in} RMS \.{:} rmState [ r ]
 \.{=}\@w{working}}%
\@pvspace{8.0pt}%
\@x{\@s{16.4} allWorking ( RMS ) \.{\defeq}}%
 \@x{\@s{32.8} \.{\land} \A\, r \.{\in} RMS \.{:} rmState [ r ]
 \.{=}\@w{working}}%
\@pvspace{8.0pt}%
\@x{\@s{16.4} isAnyPrepared ( RMS ) \.{\defeq}}%
 \@x{\@s{32.8} \.{\land} \E\, r \.{\in} RMS \.{:} rmState [ r ]
 \.{=}\@w{prepared}}%
\@pvspace{8.0pt}%
\begin{lcom}{17.5}%
...
\end{cpar}%
\end{lcom}%
\@x{\@s{16.4} tmSendCommit ( r ) \.{\defeq}}%
 \@x{\@s{32.8} \.{\land} \E\, rm \.{\in} RM \.{:} rmState [ rm ] \.{\in}
 \{\@w{prepared} ,\,\@w{committed} \}}%
\@x{\@s{32.8} \.{\land} rmState [ r ] \.{=}\@w{prepared}}%
 \@x{\@s{32.8} \.{\land} rmState \.{'} \.{=} [ rmState {\EXCEPT} {\bang} [ r ]
 \.{=}\@w{committed} ]}%
\@pvspace{8.0pt}%
\begin{lcom}{17.5}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
As described in \ensuremath{3PC}:
 all nodes answered to Prime \ensuremath{TM} Working and go to Prepared state
 one by one.
\end{cpar}%
\end{lcom}%
\@x{\@s{16.4} tmSendPrepare ( r ) \.{\defeq}}%
\@x{\@s{32.8} \.{\land} rmState [ r ] \.{=}\@w{working}}%
 \@x{\@s{32.8} \.{\land} rmState \.{'} \.{=} [ rmState {\EXCEPT} {\bang} [ r ]
 \.{=}\@w{prepared} ]}%
\@pvspace{8.0pt}%
\begin{lcom}{17.5}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
All nodes are in Working stage and Prime \ensuremath{TM} failed for a while.
 \ensuremath{BackUp} \ensuremath{TM} rise and sends ``abort'' message to
 nodes one-by-one
\end{cpar}%
\end{lcom}%
\@x{\@s{16.4} tmBackSendAbort ( r ) \.{\defeq}}%
\@x{\@s{32.8} \.{\land} rmState [ r ] \.{=}\@w{working}}%
 \@x{\@s{32.8} \.{\land} rmState \.{'} \.{=} [ rmState {\EXCEPT} {\bang} [ r ]
 \.{=}\@w{aborted} ]}%
\@pvspace{8.0pt}%
\begin{lcom}{7.5}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 Predicate. Initial state here - all \ensuremath{RM} are in working stage (Trx
 is already began)
 Primarly \ensuremath{TM} received status \ensuremath{\@w{working}} and going
 to continue transaction
 Some issue i the system, timeout for \ensuremath{TM} message. Nodes
\end{cpar}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
\ensuremath{BackUp} \ensuremath{TM
}%
\end{cpar}%
\end{lcom}%
\@x{ VCInit \.{\defeq}}%
 \@x{\@s{43.44} \.{\land} rmState \.{=} [ rm \.{\in} RM
 \.{\mapsto}\@w{working} ]}%
\@pvspace{8.0pt}%
\begin{lcom}{7.5}%
...
Next}%
state is:
 For \ensuremath{Primarly} \ensuremath{TM} - move all working nodes to
 \ensuremath{\@w{prepared}} state
 For \ensuremath{BackUp} \ensuremath{TM} - rollback \ensuremath{Trx}. Move
 nodes to \ensuremath{\@w{aborted}} state
\end{cpar}%
\end{lcom}%
\@x{ VCNext \.{\defeq}}%
\@x{\@s{39.37} \.{\lor} \E\, r \.{\in} RM \.{:}}%
 \@x{\@s{54.58} tmSendPrepare ( r ) \.{\lor} tmBackSendAbort ( r ) \.{\lor}
 tmSendCommit ( r )}%
\@pvspace{8.0pt}%
\begin{lcom}{0}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
...
\end{cpar}%
\end{lcom}%
 \@x{\@s{8.2} \.{\land} \A\, r \.{\in} RM \.{:} rmState [ r ] \.{\in}
 \{\@w{working} ,\,\@w{prepared} ,\,\@w{committed} ,\,\@w{aborted} \}}%
\@pvspace{8.0pt}%
\@x{ Consistency \.{\defeq}}%
\@pvspace{8.0pt}%
\begin{lcom}{12.5}%
\begin{cpar}{0}{F}{F}{0}{0}{}%
 A state predicate asserting that two \ensuremath{RMs} have not arrived to
 conflicting decisions.
 Prepared is the point on one-way direction to Committed stage.
 There is no Commit in \ensuremath{Git} to pull, but \ensuremath{Trx} is
 failed silently.
 However, it\mbox{'}s impossible to roll back the \ensuremath{Trx} from
 Prepared state
\end{cpar}%
\end{lcom}%
\@x{\@s{16.4} \A\, rm1 ,\, rm2 \.{\in} RM \.{:}}%
\@x{\@s{27.72} {\lnot} ( \.{\land} rmState [ rm1 ] \.{=}\@w{aborted}}%
 \@x{\@s{27.72} \.{\land} rmState [ rm2 ] \.{\in} \{\@w{prepared}
 ,\,\@w{committed} \} )}%
\@pvspace{8.0pt}%
\@x{}\bottombar\@xx{}%
\end{document}
